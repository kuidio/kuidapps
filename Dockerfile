...
RUN --mount=type=cache,target=/root/.cache/go-build \
    --mount=type=ssh \
    CGO_ENABLED=0 GOOS=${TARGETOS:-linux} GOARCH=${TARGETARCH} go build -a -o kuidapps main.go
...
#
COPY --from=builder --chown=$USERID:$USERID /workspace/kuidapps /app/
WORKDIR /app
# from now on, run as the unprivileged user
USER $USERID
ENTRYPOINT ["/app/kuidapps"]
